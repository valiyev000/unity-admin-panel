...
function MessageBox({ selectedFilter, mainContainer, conversations }) {
    ...
    return (
        ...
        >
            ...
            <ul className={styles.innerContainer} style={{height: conversations.length !== 0 ? "unset" : "calc(100% - 24px)"}}>
                ...
                )) : <motion.div layout initial={{opacity: 0}} animate={{opacity: 1}} className={styles.nullConversation}>{translation.there_is_no_conversation_for_show}</motion.div>}
            </ul>
        </motion.div>
    )
}
