...
import styles from '../styles/components/NavbarMobile.module.scss'
...
export default function NavbarMobile() {
  ...
  return (
    <nav className={`${styles.nav} ${theme === "dark" ? styles.dark : ""}`} style={{ transform: isNavOpen ? "translateX(0%)" : "translateX(-100%)" }} tabIndex={0}>
      <ForSearchMobile />
      <motion.div layout className={styles.adminTools}>{translation.admin_tools}</motion.div>
      <ul>
      ...
      <hr className={`${styles.hr} ${theme === "dark" && styles.dark}`} />
      ...
      <SwitchUser justifyContent={"center"} />
    </nav>
  )
}
